...
function getProjectWithChildren(item, callback) {
	...
	db.query('SELECT * FROM project WHERE parent_id = ' + item.id, function(err, results) {
		...
		db.query('SELECT m.name, pmx.material_quantity AS quantity FROM material as m JOIN project_material_xref as pmx ON m.id = pmx.material_id WHERE pmx.project_id = ' + item.id, function(err, results) {
			if (err) throw err;
